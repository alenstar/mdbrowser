...
RendererContainer::RendererContainer(void)
{
...
litehtml::uint_ptr RendererContainer::create_font( const litehtml::tchar_t* faceName, int size, int weight, litehtml::font_style italic, unsigned int decoration, litehtml::font_metrics* fm )
{
...
void RendererContainer::delete_font( litehtml::uint_ptr hFont )
{
...
int RendererContainer::text_width( const litehtml::tchar_t* text, litehtml::uint_ptr hFont )
{
	cairo_font* fnt = (cairo_font*) hFont;
...
void RendererContainer::draw_text( litehtml::uint_ptr hdc, const litehtml::tchar_t* text, litehtml::uint_ptr hFont, litehtml::web_color color, const litehtml::position& pos )
{
	cairo_font* fnt = (cairo_font*) hFont;
...
int RendererContainer::pt_to_px( int pt )
{
...
int RendererContainer::get_default_font_size() const
{
	return 16;
}

void RendererContainer::draw_list_marker( litehtml::uint_ptr hdc, const litehtml::list_marker& marker )
{
...
void RendererContainer::load_image( const litehtml::tchar_t* src, const litehtml::tchar_t* baseurl, bool redraw_on_ready )
{
	litehtml::tstring url;
...
void RendererContainer::get_image_size( const litehtml::tchar_t* src, const litehtml::tchar_t* baseurl, litehtml::size& sz )
{
	litehtml::tstring url;
	make_url(src, baseurl, url);
...
void RendererContainer::draw_background( litehtml::uint_ptr hdc, const litehtml::background_paint& bg )
{
	cairo_t* cr = (cairo_t*) hdc;
	cairo_save(cr);
	apply_clip(cr);
...
void RendererContainer::make_url(const litehtml::tchar_t* url,	const litehtml::tchar_t* basepath, litehtml::tstring& out)
{
	out = url;
}

void RendererContainer::add_path_arc(cairo_t* cr, double x, double y, double rx, double ry, double a1, double a2, bool neg)
{
	if(rx > 0 && ry > 0)
	{
...
void RendererContainer::draw_borders(litehtml::uint_ptr hdc, const litehtml::borders& borders, const litehtml::position& draw_pos, bool root)
{
	cairo_t* cr = (cairo_t*) hdc;
	cairo_save(cr);
	apply_clip(cr);
...
void RendererContainer::transform_text(litehtml::tstring& text, litehtml::text_transform tt)
{

}

void RendererContainer::set_clip( const litehtml::position& pos, const litehtml::border_radiuses& bdr_radius, bool valid_x, bool valid_y )
{
...
void RendererContainer::del_clip()
{
	if(!m_clips.empty())
	{
		m_clips.pop_back();
...
void RendererContainer::apply_clip( cairo_t* cr )
{
...
void RendererContainer::draw_ellipse( cairo_t* cr, int x, int y, int width, int height, const litehtml::web_color& color, int line_width )
{
	if(!cr) return;
	cairo_save(cr);
...
void RendererContainer::fill_ellipse( cairo_t* cr, int x, int y, int width, int height, const litehtml::web_color& color )
{
	if(!cr) return;
	cairo_save(cr);
...
void RendererContainer::clear_images()
{
...
const litehtml::tchar_t* RendererContainer::get_default_font_name() const
{
	return "Times New Roman";
}

std::shared_ptr<litehtml::element>	RendererContainer::create_element(const litehtml::tchar_t *tag_name,
...
void RendererContainer::rounded_rectangle( cairo_t* cr, const litehtml::position &pos, const litehtml::border_radiuses &radius )
{
	cairo_new_path(cr);
...
void RendererContainer::draw_pixbuf(cairo_t* cr, const Glib::RefPtr<Gdk::Pixbuf>& bmp, int x,	int y, int cx, int cy)
{
	cairo_save(cr);
...
cairo_surface_t* RendererContainer::surface_from_pixbuf(const Glib::RefPtr<Gdk::Pixbuf>& bmp)
{
...
void RendererContainer::get_media_features(litehtml::media_features& media) const
{
...
void RendererContainer::get_language(litehtml::tstring& language, litehtml::tstring& culture) const
{
	language = _t("en");
	culture = _t("");
}

void RendererContainer::link(const std::shared_ptr<litehtml::document> &ptr, const litehtml::element::ptr& el)
{
